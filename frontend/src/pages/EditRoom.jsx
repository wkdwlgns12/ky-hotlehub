...
const EditRoom = () => {
  ...
  const fetchHotelAndRoom = async () => {
    try {
      ...
      setFormData({
        ...
        isOnSale: room.isOnSale || false,
        capacity: room.capacity || 2,
        inventory: room.inventory || 10,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.put(`/hotels/rooms/${roomId}`, roomData);
      alert('객실이 수정되었습니다.');
      navigate('/business/dashboard');
    } catch (err) {
      setError(err.response?.data?.message || '객실 수정에 실패했습니다.');
    } finally {
      setLoading(false);
    }
  };
              ...
              <div className="images-preview">
                {formData.images.map((image, index) => (
                  <div key={index} className="image-item">
                    <img src={image.url} alt={image.alt || `Room ${index + 1}`} />
                    <button
                      type="button"
